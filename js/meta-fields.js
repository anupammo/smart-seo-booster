/**
 * Smart SEO Booster - Meta Fields Admin JavaScript
 * 
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        // Load SEO Analysis with proper nonce (from localized script data)
        window.loadSeoAnalysis = function() {
            var postId = $('#post_ID').val();
            if (!postId) return;
            
            $('#seo-analysis-content').html('<p>Loading comprehensive SEO analysis...</p>');
            
            $.post(smartSeoMeta.ajaxUrl, {
                action: 'get_full_seo_report',
                post_id: postId,
                nonce: smartSeoMeta.nonce
            }, function(response) {
                if (response.success) {
